...
import { Settings, Maximize2, Download, RefreshCw, Trash2, Share2, Activity } from 'lucide-react';
        ...
        <div className={`lg:col-span-8 space-y-6 ${isFullscreen ? 'fixed inset-0 z-[100] bg-cyber-bg p-6' : ''}`}>
          ...
          {!isFullscreen && (
            <div className="flex gap-4">
               ...
               <GlassPanel className="flex-1 py-4 flex items-center justify-center gap-4 hover:bg-white/5 cursor-pointer transition-colors border-dashed group">
                  <Activity className="w-4 h-4 text-zinc-500 group-hover:text-cyber-purple" />
                  <span className="text-xs font-mono text-zinc-400 group-hover:text-white">Neural Protocol</span>
               </GlassPanel>
            </div>
          )}
        </div>
